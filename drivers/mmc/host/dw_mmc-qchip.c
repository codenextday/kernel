...
#include "dw_mmc-qchip.h"

struct dw_mci_qchip_priv_data {
...
static int dw_mci_qchip_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	struct dw_mci *host = slot->host;
...
#endif
static int dw_mci_qchip_parse_dt(struct dw_mci *host)
{
	struct device_node *np = host->dev->of_node;
	struct device_node *node;
	struct dw_mci_qchip_priv_data *priv;
	int ret;
...
static unsigned long qchip_dwmmc_caps[3] = {
	MMC_CAP_CMD23,
	MMC_CAP_CMD23,
	MMC_CAP_CMD23,
};

static const struct dw_mci_drv_data qchip_drv_data = {
	.caps			= qchip_dwmmc_caps,
	.num_caps		= ARRAY_SIZE(qchip_dwmmc_caps),
	/*.execute_tuning		= dw_mci_qchip_execute_tuning,*/
	/*.prepare_hs400_tuning	= dw_mci_zx_prepare_hs400_tuning,*/
	.parse_dt               = dw_mci_qchip_parse_dt,
};

static const struct of_device_id dw_mci_qchip_match[] = {
	{ .compatible = "qchip,qchip-dwmmc", .data = &qchip_drv_data},
	{},
};
MODULE_DEVICE_TABLE(of, dw_mci_qchip_match);

static int dw_mci_qchip_probe(struct platform_device *pdev)
{
	const struct dw_mci_drv_data *drv_data;
	const struct of_device_id *match;

	match = of_match_node(dw_mci_qchip_match, pdev->dev.of_node);
	drv_data = match->data;

	return dw_mci_pltfm_register(pdev, drv_data);
}

static const struct dev_pm_ops dw_mci_qchip_dev_pm_ops = {
...
static struct platform_driver dw_mci_qchip_pltfm_driver = {
	.probe		= dw_mci_qchip_probe,
	.remove		= dw_mci_pltfm_remove,
	.driver		= {
		.name		= "dwmmc_qchip",
		.of_match_table	= dw_mci_qchip_match,
		.pm		= &dw_mci_qchip_dev_pm_ops,
	},
};

module_platform_driver(dw_mci_qchip_pltfm_driver);

MODULE_DESCRIPTION("QCHIP emmc/sd driver");
